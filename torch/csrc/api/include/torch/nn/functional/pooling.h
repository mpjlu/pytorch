...
inline Tensor avg_pool1d(const Tensor& input,
                         ...
                         ExpandingArray<D> stride,
                         ExpandingArray<D> padding = 0,
...
inline std::tuple<Tensor, Tensor> adaptive_max_pool1d_with_indices(
  const Tensor& input, ExpandingArray<D> output_size) {
...
inline Tensor lp_pool1d(
  ...
  Tensor out = avg_pool1d(
    input.pow(norm_type),
    AvgPool1dOptions(kernel_size).stride(stride).padding(0).ceil_mode(ceil_mode));
...
inline Tensor lp_pool2d(
  ...
  Tensor out = avg_pool2d(
    input.pow(norm_type),
    AvgPool2dOptions(kernel_size).stride(stride).padding(0).ceil_mode(ceil_mode));
