...
inline Tensor local_response_norm(
    ...
    if (dim == 3) {
      div = pad(div, PadOptions({0, 0, size / 2, (size - 1) / 2}));
      div = avg_pool2d(div, AvgPool2dOptions({size, 1}).stride(1)).squeeze(1);
    } else {
      auto sizes = input.sizes();
      div = div.view({sizes[0], 1, sizes[1], sizes[2], -1});
      div = pad(div, PadOptions({0, 0, 0, 0, size / 2, (size - 1) / 2}));
      div = avg_pool3d(div, AvgPool3dOptions({size, 1, 1}).stride(1)).squeeze(1);
      div = div.view(sizes);
    }
