...
Tensor ELUImpl::forward(Tensor input) {
  return F::elu(input, options.alpha(), options.inplace());
}
...
Tensor SELUImpl::forward(Tensor input) {
  return F::selu(input, options.inplace());
}
...
Tensor HardshrinkImpl::forward(const Tensor& input) {
  return F::hardshrink(input, options.lambda());
}
...
Tensor HardtanhImpl::forward(Tensor input) {
  return F::hardtanh(input, options.min_val(), options.max_val(), options.inplace());
}
...
Tensor LeakyReLUImpl::forward(Tensor input) {
  return F::leaky_relu(input, options.negative_slope(), options.inplace());
}
...
Tensor SoftmaxImpl::forward(const Tensor& input) {
  return F::softmax(input, options.dim());
}
...
Tensor SoftminImpl::forward(const Tensor& input) {
  return F::softmin(input, options.dim());
}
...
Tensor LogSoftmaxImpl::forward(const Tensor& input) {
  return F::log_softmax(input, options.dim());
}
...
Tensor ReLUImpl::forward(Tensor input) {
  return F::relu(input, options.inplace());
}
...
Tensor ReLU6Impl::forward(Tensor input) {
  return F::relu6(input, options.inplace());
}
...
Tensor RReLUImpl::forward(Tensor input) {
  return F::rrelu(input, options.lower(), options.upper(), options.inplace(), is_training());
}
...
Tensor CELUImpl::forward(Tensor input) {
  return F::celu(input, options.alpha(), options.inplace());
}
...
Tensor SoftplusImpl::forward(const Tensor& input) {
  return F::softplus(input, options.beta(), options.threshold());
}
...
Tensor SoftshrinkImpl::forward(const Tensor& input) {
  return F::softshrink(input, options.lambda());
}
...
Tensor ThresholdImpl::forward(Tensor input) {
  return F::threshold(input, options.threshold(), options.value(), options.inplace());
}
